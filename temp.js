...
onLoad(); // Call onLoad function when the script loads

function onLoad() {
  let bagItemsStr = localStorage.getItem('bagItems'); 
  bagItems = bagItemsStr ? JSON.parse(bagItemsStr) : []; // Initialize bagItems with stored data or an empty array
  displayItems();
  bagCount();
}
...
function displayItems() {
  ...
  let innerHTML = ``;
  items.forEach(item => { // Assuming `items` array is defined elsewhere in your code
    innerHTML += `
